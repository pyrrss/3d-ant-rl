...
def select_model_saved() -> tuple[bool, str | None, str | None]:
    ...
    model_saved = questionary.select(
        "¿Desea cargar un modelo existente?: ", choices=["True", "False"]
    ).ask()
    model_saved = True if model_saved == "True" else False

    name_model = questionary.select(
        "Seleccione un modelo a entrenar: ", choices=MODELS
    ).ask()

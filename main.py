...
def main() -> None:
    ...
    if not model_saved: 
        re = ReinforcementLearningModels(eval_env=eval_env, train_env=train_env, model=name_model)
        ...
        if os.path.exists(model_file):
            ...
            print(f"El modelo {model_file} no existe (Verifique que esté guardado este archivo).")
            return
        
    scores = evaluate_model(model, 1_00, render=True)
